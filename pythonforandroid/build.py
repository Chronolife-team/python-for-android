...
def get_ndk_platform_dir(ndk_dir, ndk_api, arch):
    ...
    ndk_platform = join(
        ndk_dir, "platforms", "android-{}".format(ndk_api), platform_dir
    )
...
def get_toolchain_versions(ndk_dir, arch):
    ...
    if isdir(toolchain_path):
        ...
        toolchain_versions = [
            split(path)[-1][len(toolchain_prefix) + 1 :] for path in toolchain_contents
        ]
    else:
...
class Context:
    ...
    def libs_dir(self):
        ...
        directory = join(
            self.build_dir, "libs_collections", self.bootstrap.distribution.name
        )
    ...
    def javaclass_dir(self):
        ...
        directory = join(
            self.build_dir, "javaclasses", self.bootstrap.distribution.name
        )
        ensure_dir(directory)
        return directory
    ...
    def ndk_api(self):
        ...
        if self._ndk_api is None:
            raise ValueError(
                "Tried to access ndk_api but it has not "
                "been set - this should not happen, something "
                "went wrong!"
            )
    ...
    def prepare_build_environment(
        self, user_sdk_dir, user_ndk_dir, user_android_api, user_ndk_api
    ):
        ...
        if user_android_api:
            ...
            info(
                "Getting Android API version from user argument: {}".format(android_api)
            )
        ...
        if toolchain_versions:
            info(
                "Found the following toolchain versions: {}".format(toolchain_versions)
            )
            info(
    ...
    def has_package(self, name, arch=None):
        ...
        if (name.find("/") >= 0 or name.find("\\") >= 0) and name.find(
            "://"
        ) < 0:  # (:// would indicate an url)
...
def build_recipes(
    build_order, python_modules, ctx, project_dir, ignore_project_setup_py=False
):
    ...
    for arch in ctx.archs:
        ...
        if not ctx.python_recipe:
            ...
            warning(
                "Context's python recipe found, " "skipping biglink (will this work?)"
            )
    ...
    run_pymodules_install(
        ctx, python_modules, project_dir, ignore_setup_py=ignore_project_setup_py
    )


def project_has_setup_py(project_dir):
    return project_dir is not None and (
        exists(join(project_dir, "setup.py"))
        or exists(join(project_dir, "pyproject.toml"))
    )
...
def run_setuppy_install(ctx, project_dir, env=None):
    ...
    with current_directory(project_dir):
        info(
            "got setup.py or similar, running project install. "
        ...
        try:
            ...
            info(
                "Populating venv's site-packages with " "ctx.get_site_packages_dir()..."
            )
            ...
            venv_site_packages_dir = os.path.normpath(
                os.path.join(
                    ...
                    [
                        f
                        for f in os.listdir(os.path.join(ctx.build_dir, "venv", "lib"))
            ...
            for f in os.listdir(ctx_site_packages_dir):
                ...
                if not os.path.exists(os.path.join(venv_site_packages_dir, f)):
                    if os.path.isdir(full_path):
                        ...
                        shutil.copy2(full_path, os.path.join(venv_site_packages_dir, f))
            ...
            info(
                "Copying additions resulting from setup.py back "
                "into ctx.get_site_packages_dir()..."
            )
            new_venv_additions = []
            for f in set(os.listdir(venv_site_packages_dir)) - set(
                previous_venv_contents
            ):
                new_venv_additions.append(f)
                full_path = os.path.join(venv_site_packages_dir, f)
                if os.path.isdir(full_path):
                    shutil.copytree(full_path, os.path.join(ctx_site_packages_dir, f))
                else:
                    shutil.copy2(full_path, os.path.join(ctx_site_packages_dir, f))
...
def run_pymodules_install(ctx, modules, project_dir=None, ignore_setup_py=False):
    ...
    if not modules and (
        ignore_setup_py or project_dir is None or not project_has_setup_py(project_dir)
    ):
    ...
    if (
        ...
        and project_has_setup_py(project_dir)
